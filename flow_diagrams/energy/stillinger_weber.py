...
from jax_md.energy import stillinger_weber
...
def fd_multiplicative_isotropic_cutoff(
    ...
    def cutoff_fn(dr, *args, **kwargs):
        print(smooth, shift)
        if smooth:
            print("dr = ", dr, type(fn), args, kwargs )
...
def fd_stillinger_weber_neighbor_list(
    ...
    **neighbor_kwargs
    ...
    if soft_core:
        func = lambda dr, sigma, epsilon,: soft_stillinger_weber(
            dr=dr, sigma=sigma, epsilon=epsilon, lambda_lj=lambda_lj
        )
    else:
        func = stillinger_weber
